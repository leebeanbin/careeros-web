...
const matchItems = [
  { title: "백엔드 방향성 정리", context: "관심 역할", score: 92, color: "#22c55e", active: false },
  { title: "풀스택 전환 준비", context: "목표 역할", score: 87, color: "#22c55e", active: true },
  { title: "이력서 보완 과제", context: "다음 행동", score: 74, color: "#eab308", active: false },
  { title: "면접 준비 루틴", context: "지원 전 점검", score: 68, color: "#eab308", active: false },
];
...
const sidebarItems = ["대시보드", "관심 역할", "나의 흐름", "이력서", "GitHub", "AI 어드바이저"];
...
export default function HeroSection() {
  return (
    <MarketingSection className="relative overflow-hidden pt-[132px] lg:pt-[180px]">
      ...
      <div
        aria-hidden="true"
        className="pointer-events-none absolute left-0 right-0 top-[72px] h-px bg-[linear-gradient(90deg,transparent,rgba(129,140,248,0.5),transparent)]"
      />
      <MarketingContainer className="relative">
        <div className="mb-6 flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <MarketingHeading size="hero" className="max-w-[860px]">
            내 커리어를 함께 정리하는<br />AI 어시스턴트
          </MarketingHeading>
        ...
        <div className="flex min-h-[520px] min-w-[820px] overflow-hidden rounded-t-xl border border-b-0 border-[rgba(255,255,255,0.1)] bg-[#0F1011] shadow-[0_40px_120px_rgba(0,0,0,0.5)]">
          {/* Sidebar */}
          <div className="w-[230px] shrink-0 border-r border-[rgba(255,255,255,0.06)]
            ...
            <div className="mt-auto rounded-lg border border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.03)] p-3">
              ...
              {activityFeed.map((item) => (
                <div key={item.label} className="flex items-center justify-between border-t border-[rgba(255,255,255,0.05)] py-1.5 first:border-t-0">
          ...
          <div className="flex-1 border-r border-[rgba(255,255,255,0.06)] bg-[#0D0E0F] p-4">
            ...
            {matchItems.map((item) => (
              <div key={item.context}
                   className={`mb-1 grid grid-cols-[1fr_64px_54px] items-center rounded-md px-3 py-2 text-[13px]
                     ${item.active ? "border border-[rgba(129,140,248,0.3)] bg-[rgba(129,140,248,0.08)]" : "border border-transparent"}`}>
                <div>
                ...
                <div className="h-1.5 rounded-full bg-[rgba(255,255,255,0.08)]">
                  <div className="h-1.5 rounded-full bg-[#818cf8]" style={{ width: `${item.score}%` }} />
                </div>
